...
@section('title')
    {{ trans('admin/facillities.page-title') }}
@endsection
        ...
        <div class="box-header with-border">
            ...
            <div class="box-body">
                ...
                <table class="table table-bordered table-hover dataTable" id="table">
                    <thead>
                    <tr role="row">
                        <th>{{ trans('admin/facillities.id') }}</th>
                        <th>{{ trans('admin/facillities.title') }}</th>
                        <th>{{ trans('admin/facillities.info') }}</th>
                        <th></th>
                    </tr>
                    ...
                    @foreach ($facillities as $facillity)
                        <tr role="row" class="odd">
                            <td>{{ $facillity -> id }}</td>
                            <td>{{ $facillity -> title }}</td>
                            <td class="text-center">
                                <a class="btn btn-primary " id="show-btn" data-toggle="modal" data-target="#modal-show1"
                                   data-id="{{ $facillity -> id }}" data-title="{{ $facillity -> title }}"
                                   data-is_active="{{ $facillity -> is_active }}" data-info="{{ $facillity -> info  }}"><i class="fa fa-search-plus" aria-hidden="true"></i></a>
                                <a class="btn btn-warning " id="edit-btn" data-toggle="modal" data-target="#modal-edit"
                                   data-id="{{ $facillity -> id }}" data-title="{{ $facillity -> title }}"
                                   data-is_active="{{ $facillity -> is_active }}" data-info="{{ $facillity -> info }}"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                <a class="btn btn-danger " id="delete-btn" data-toggle="modal"
                                   data-target="#modal-delete" data-id="{{ $facillity -> id }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">{{ trans('admin/facillities.add-facillity-header') }}</h4>
                        </div>
                        <div class="modal-body">
                            {!! Form::open(array('action' => array('FacillitiesController@post'))) !!}
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span></button>
                            <h4 class="modal-title">{{ trans('admin/facillities.edit-facillity-header') }}</h4>
                        </div>
                        <div class="modal-body">
                            {!! Form::open(array('route' => 'admin_facillities_update')) !!}
                            {!! Form::label('title', 'Όνομα') !!}
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="text-center">
                                <h3>{{ trans('admin/facillities.delete-facillity-header') }} </h3>
                                {!! Form::open(array('route' => 'admin_facillities_delete')) !!}
                                {{ Form::hidden('id', 'null', ['id' => 'ids']) }}
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span></button>
                            <h4 class="modal-title" id="modal-title">{{ trans('admin/facillities.show-facillity-header') }}</h4>
                        </div>
                        <div class="modal-body">
                            {!! Form::label('title', 'Όνομα') !!}
