    ...
    $(document).ready(function() {
            ...
            $('#tweetiefeed').twittie(
                {
                    ...
                    'dateFormat' : '%b-%d-%Y',
                    'template'   : '<span class="csi-thumb">{{avatar}}</span><div class="csi-tweet-content"><span class="csi-date">{{date}}</span><p class="csi-message">{{tweet}}<a href="{{url}}" target="_blank">{{url}}</a></p><span class="csi-author">{{user_name}}</span></div>'
        ...
        if($('#csi-typed-string').length){
            $('#csi-typed-string').typed({
                strings: ["Delicious Food", "Quality Food","different Food"],
                // typing speed
                typeSpeed: 60,
        ...
        $bookingForm.validate({
            submitHandler: function (form) {
                ...
                var $form = $(form);
                $.ajax({
                    url         : csi_path + '/assets/php/booking.php',
                    type        : 'post',
